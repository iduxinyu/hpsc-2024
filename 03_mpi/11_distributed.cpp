...
int main(int argc, char** argv) {
  ...
  MPI_Win win;
  MPI_Win_create(jbody,(N*sizeof(double)*5)/size,5*sizeof(double), MPI_INFO_NULL,MPI_COMM_WORLD,&win);
  ...
  for(int irank=0; irank<size; irank++) {
   ...
   //
     for(int i=0;i<N/size;i++)
	     temp[i]=jbody[i];
     MPI_Win_fence(0,win);
     MPI_Put(temp,N/size,MPI_BODY,send_to,0,N/size,MPI_BODY,win);
     MPI_Win_fence(0,win); 
    for(int i=0; i<N/size; i++) {
